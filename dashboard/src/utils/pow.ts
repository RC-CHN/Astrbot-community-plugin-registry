export async function solveRegistrationPow(
  ...
  salt: string,
  difficulty: number,
  options: { timeoutMs?: number; onProgress?: (attempts: number) => void } = {},
) {
  ...
  while (Date.now() - startedAt <= timeoutMs) {
    ...
    const digest = await sha256(`${challengeId}:${salt}:${nonce}`)
  ...
  throw new Error('PoW 计算超时，请稍后重试')
}
...
function leadingZeroBits(data: Uint8Array) {
  let bits = 0
...
function sha256Fallback(message: Uint8Array) {
  ...
  for (let i = 0; i < words.length; i += 16) {
    ...
    for (let j = 0; j < 64; j += 1) {
      ...
      const ch = (e & f) ^ (~e & g)
      const temp1 = add32(h, s1, ch, sha256K[j], w[j])
